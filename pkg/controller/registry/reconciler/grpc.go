...
func (c *GrpcRegistryReconciler) ensureUpdatePod(source grpcCatalogSourceDecorator) error {
	...
	for _, pod := range currentUpdatePods {
...
// containerReady returns true if the given Pod has a ready status for its container.
func containerReady(pod *corev1.Pod) bool {
