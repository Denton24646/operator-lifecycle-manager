...
func newOperatorWithConfig(ctx context.Context, config *operatorConfig) (*Operator, error) {
	...
	op := &Operator{
		...
		operandDeleter: &finalizers.OperandDeleter{
			...
			ClusterServiceVersionLister:    lister.OperatorsV1alpha1().ClusterServiceVersionLister(),
			OperatorGroupLister:            lister.OperatorsV1().OperatorGroupLister(),
...
func (a *Operator) syncClusterServiceVersion(obj interface{}) (syncError error) {
	...
	if syncError == nil {
		if outCSV == nil {
			outCSV = clusterServiceVersion.DeepCopy()
		}

		if ok, err := a.operandDeleter.DeleteOperands(outCSV, &outCSV.Status.Cleanup); err != nil {
			...
			logger.Info("finished processing operand deletion finalizer")
		}
	}
	...
	if !(outCSV.Status.LastUpdateTime.Equal(clusterServiceVersion.Status.LastUpdateTime) &&
		...
		cmp.Equal(outCSV.Status.Cleanup, clusterServiceVersion.Status.Cleanup)) {
